...
void f_filltspan(std::vector<double> &v, double tsta, double tend, int framerate){
    ...
    for (unsigned int i=1; i<v.size(); i++){
            while ((v[i-1]+(1.0/double(framerate)))<tend){
                v[i]=v[i-1]+(1.0/double(framerate));
            }
    }
...
void f_printmodelparameters(double g, double m1, double m2, double l){
    // #################
    // Your code here...
     std::cout << "\n  Expected values:\n\t gravitational constant: "<<g<<" mass of wagon: "<<m1<< " mass of pendulum: "<<m2<<" length of pendulum: "<<l<<"\n";
...
void f_veccpy(std::vector<double> &v,  std::vector<double> &w){
